...
if ($_GET[action]=="dump" && $_GET[id]<$number_of_sites) {
	...
	echo "<tr style='background-color:#a2a5a7;text-align: left;'>
		...
		<th>SORT BY<br />$request_uri_label</th>
	...
	if($stmt = $mysqli->prepare
			("SELECT id,timestamp,php_self,remote_addr,concat(http_host,request_uri) as url, http_referer,http_user_agent as number FROM $tablename[$siteid]
			ORDER BY DATE(timestamp),url,remote_addr;"))
	{
		//$stmt->bind_param("s",$tablename[$siteid]);
		$stmt->execute();
		//$stmt->store_result();
		$numrows = $stmt->num_rows;
		$stmt->bind_result($id,$timestamp,$php_self,$remote_addr,$url,$http_referer,$http_user_agent);
		$i=1;
		$countt=$i;
		while($stmt->fetch())
		{
			...
			{
				$iminus=$i-1;
				//echo "<tr><td>.</td><td>.</td><td>.</td><td>.</td><td>.</td><td>"."count = ".$countt."</td><td>.</td><td>.</td><td>.</td><td>.</tr>";
			...
			if ($Datumm<>$Datum && $Datumm<>"" && $Datum<>"")
				//{echo "<tr><td>-----</td><td>-----</td><td>-----</td><td>-----</td><td>"."date = ".$Datum."</td><td>-----</td><td>-----</td><td>-----</td><td>-----</td><td></tr>";}
			{
				echo "<tr>";
			...
			<td style='word-break: break-all; word-wrap: normal;'>$url</td>
