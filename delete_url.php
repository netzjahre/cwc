...
$rows_del = 0;
		...
		if(isset ($_POST['url']))
			{
			$urlnum = htmlentities(stripslashes($_POST['url']), ENT_QUOTES, 'UTF-8');
			$urlnumber = "%$urlnum%";
				if($stmt = $mysqli->prepare("DELETE FROM `$tablename[$siteid]` WHERE request_uri LIKE ?"))
					{
					$stmt->bind_param("s", $urlnumber);
					$stmt->execute();
